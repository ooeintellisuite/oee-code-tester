...
describe('checkTsConfig', () => {
    it('should create a tsconfig.json file if it does not exist', () => {
      ...
      checkTsConfig();
      expect(logSpy).toHaveBeenCalledWith( chalk.yellow('🔍 Checking tsconfig.json...'));
      expect(logSpy ).toHaveBeenCalledWith(chalk.red('❌ No tsconfig.json file found! Creating one...'));
      expect(vi.mocked(createTsConfig)).toHaveBeenCalled();
      expect(logSpy ).toHaveBeenCalledWith(chalk.green('✔ tsconfig.json has been created.'));
      logSpy.mockRestore();
    });
    it('should fix the tsconfig.json file if it is incorrect', () => {
      ...
      checkTsConfig();
      expect(logSpy).toHaveBeenCalledWith(chalk.yellow('🔍 Checking tsconfig.json...'));
      expect(logSpy).toHaveBeenCalledWith(chalk.red('❌ tsconfig.json is incorrect! Fixing it...'));
      expect(vi.mocked(fixTsConfig)).toHaveBeenCalled();
      expect(logSpy ).toHaveBeenCalledWith(chalk.green('✔ tsconfig.json has been updated to the correct configuration.'));
      logSpy.mockRestore();
    });
    it('should do nothing if tsconfig.json is correct', () => {
      ...
      checkTsConfig();
      expect(logSpy).toHaveBeenCalledWith(chalk.yellow('🔍 Checking tsconfig.json...'));
      expect(logSpy).toHaveBeenCalledWith(chalk.green('✔ tsconfig.json matches the expected configuration.'));
