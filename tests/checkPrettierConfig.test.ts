...
import { createPrettierConfig, fixPrettierConfig, isPrettierConfigValid, readPrettierConfig } from '../utils/prettierUtils';
...
describe('checkPrettierConfig', () => {
  it('should create a new .prettierrc file if it does not exist', () => {
     ...
     expect(logSpy ).toHaveBeenCalledWith(chalk.red('❌ .prettierrc not found! Creating one...'));
     expect(vi.mocked(createPrettierConfig)).toHaveBeenCalled(); // Should create a new .gitignore
     expect(logSpy ).toHaveBeenCalledWith(chalk.green('✔ .prettierrc has been created.'));
     logSpy.mockRestore();
  });
  ...
  it('should fix .prettierrc if it contains incorrect settings', () => {
    ...
    checkPrettierConfig();
    expect(logSpy).toHaveBeenCalledWith(chalk.yellow('🔍 Checking Prettier configuration...'));
    expect(logSpy).toHaveBeenCalledWith(chalk.red('❌ .prettierrc settings are incorrect! Fixing it...'));
    expect(vi.mocked(fixPrettierConfig)).toHaveBeenCalled(); // Should fix the incorrect .gitignore
    expect(logSpy ).toHaveBeenCalledWith(chalk.green('✔ .prettierrc has been updated.'));
  ...
  it('should run Prettier if the config is valid', () => {
    ...
    expect(execSync).toHaveBeenCalledWith('npx prettier --write .', { stdio: 'inherit' });
    expect(logSpy).toHaveBeenCalledWith(chalk.green('✔ Prettier formatting completed.'));
