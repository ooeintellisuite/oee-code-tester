...
describe('checkDependencies', () => {
  it('should return false if package.json is missing', () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(
      chalk.yellow('🔍 Checking dependencies...'),
    );
    expect(logSpy).toHaveBeenCalledWith(chalk.red('❌ No package.json found!'));
    expect(logSpy).toHaveBeenCalledWith(
      chalk.red('👉 Fix missing package.json: npm init -y'),
    );
  ...
  it('should return false if some required dependencies are missing', () => {
    ...
    vi.mocked(areDependenciesValid).mockReturnValue({
      valid: false,
      missing: ['vitest'],
    });
    const logSpy = vi.spyOn(console, 'log');
    ...
    const result = checkDependencies();
    expect(logSpy).toHaveBeenCalledWith(
      chalk.red('❌ Missing dependency: vitest'),
    );
    expect(logSpy).toHaveBeenCalledWith(
  ...
  it('should return true if all required dependencies are installed', () => {
    ...
    vi.mocked(getInstalledDependencies).mockReturnValue(
      new Set(['react', 'vitest']),
    );
    vi.mocked(areDependenciesValid).mockReturnValue({
      valid: true,
      missing: [''],
